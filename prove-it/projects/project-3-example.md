...
Gate evidence for that contract and checked code: [`control/receipts/p3-regression.json`](project-1-example.md#what-the-first-run-actually-did)
...
| Compound | Here is the one change I kept; here is the later run it improved | [the dedupe regression case and its later gate refusal](project-2-example.md#the-evidence-that-came-back) |

Run shape (how many agents worked, how they connected, and why): one agent ran the task. A second agent would add coordination without an independent check.
